...
# Detect OS and use appropriate method to run codex in a pseudo-TTY
# On macOS/Linux: use `script` command for line-buffering
# On Windows (Git Bash/WSL): use direct execution with stdbuf or fallback to pipe
os_type="$(uname -s)"

if [[ "$os_type" == "Darwin" ]] || [[ "$os_type" == "Linux" ]]; then
  # macOS/Linux: Use `script` to run codex in a pseudo-TTY so it line-buffers its JSONL output.
  ...
  # Windows (Git Bash/WSL/MSYS2): Use stdbuf if available, otherwise direct pipe
  # stdbuf forces line-buffering even when stdout is a pipe
  if command -v stdbuf >/dev/null 2>&1; then
    stdbuf -oL "${cmd[@]}" < "$prompt_file" 2>"$stderr_file" \
      | while IFS= read -r line; do
        # Strip terminal artifacts (carriage return, ^D EOF marker)
        cleaned="${line//$'\r'/}"
        cleaned="${cleaned//$'\004'/}"
    ...
    # Fallback: direct execution without pseudo-TTY (may have buffering issues but will work)
